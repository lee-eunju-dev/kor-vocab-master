...
function PhoneFrame({ label, className, children, ...props }: PhoneFrameProps) {
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div
        className={cn(
          "relative w-full max-w-[280px] overflow-hidden rounded-3xl border-6 border-ink bg-card shadow-chunky-ink",
          className
        )}
        {...props}
      >
        <div className="absolute inset-x-0 top-0 z-20 flex justify-center pt-2">
          <div className="h-1.5 w-14 rounded-full bg-ink/30" />
        </div>
        <div className="relative isolate flex min-h-[500px] flex-col overflow-hidden pt-6">{children}</div>
      </div>
    </div>
  )
}
